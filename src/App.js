...
const C={
  ...
  nineBox:{
    ...
    botLeft:"#93C5FD",botMid:"#60A5FA",botRight:"#3B82F6",
  },
};
...
const computeWeightedComp=(compName,compDetail,scaleVal,dirWeights)=>{
  const detail=compDetail[compName];
  if(!detail)return null;
  const items=Object.entries(detail).map(([dir,val])=>({dir,score:scaleVal(val)}));
  const valid=items.filter(it=>(dirWeights[it.dir]||0)>0);
  if(!valid.length)return null;
  const tw=valid.reduce((s,it)=>s+(dirWeights[it.dir]||0),0);
  return tw===0?null:valid.reduce((s,it)=>s+it.score*(dirWeights[it.dir]||0),0)/tw;
};
...
const NINE_BOX_LABELS={
  ...
  cells:[
    [{label:"Enigma",desc:"Alto potencial, bajo desempeño"},{label:"Futura estrella",desc:"Alto potencial, desempeño medio"},{label:"Estrella",desc:"Alto potencial, alto desempeño"}],
    [{label:"Dilema",desc:"Potencial medio, bajo desempeño"},{label:"Contribuidor clave",desc:"Potencial y desempeño medios"},{label:"Alto desempeño",desc:"Potencial medio, alto desempeño"}],
    [{label:"Riesgo",desc:"Bajo potencial, bajo desempeño"},{label:"Efectivo",desc:"Bajo potencial, desempeño medio"},{label:"Profesional sólido",desc:"Bajo potencial, alto desempeño"}],
  ],
  cellColors:[
    ["#DBEAFE","#93C5FD","#3B82F6"],
    ["#FEF3C7","#DBEAFE","#93C5FD"],
    ["#FEE2E2","#FEF3C7","#DBEAFE"],
  ],
  cellTextColors:[
    ["#1E40AF","#1E3A8A","#FFFFFF"],
    ["#92400E","#1E40AF","#1E3A8A"],
    ["#991B1B","#92400E","#1E40AF"],
  ],
};

const getNineBoxCell=(desempeno,potencial,max)=>{
  const third=max/3;
  const col=desempeno<=third?0:desempeno<=third*2?1:2;
  const row=potencial<=third?2:potencial<=third*2?1:0;
  return{row,col};
};

const NineBoxView=({data,potData,config,potConfig,scaleVal,potScaleVal,mx,potMx,users,mapping,potMapping,dirWeights,potDirWeights,onSelectUser})=>{
  const tt=useTooltip();
  ...
  const nineBoxData=useMemo(()=>{
    ...
    potData.total.forEach(r=>{
      ...
      if(u)potUsersExact[u]=r;
      if(n)potUsersExact[n]=r;
      if(u)potUsersNorm[norm(u)]=r;
      if(n)potUsersNorm[norm(n)]=r;
    });

    return users.map(u=>{
      const ud=buildUserData(data,u.username);
      if(!ud)return null;
      ...
      const cell=getNineBoxCell(desScore,potScore,mx);
      return{...u,desScore,potScore,cell,ud,potUd};
    }).filter(Boolean);
  ...
  const totalPeople=nineBoxData.length;
  const unmatchedCount=users.length-totalPeople;

  return(<div style={{maxWidth:960,margin:"0 auto",padding:24}}><tt.Tip/>
    <h2 style={{fontSize:20,fontWeight:700,color:C.text,margin:"0 0 6px",letterSpacing:"-0.02em"}}>9-Box: Potencial vs Desempeño</h2>
    <p style={{fontSize:13,color:C.textSec,margin:"0 0 20px"}}>{totalPeople} personas ubicadas{unmatchedCount>0?` · ${unmatchedCount} sin match entre ambos sheets`:""}</p>

    <div style={{display:"grid",gridTemplateColumns:"repeat(3,1fr)",gap:14,marginBottom:20}}>
      {[["Personas",totalPeople],["Prom. Desempeño",fmt(nineBoxData.length?nineBoxData.reduce((s,u)=>s+u.desScore,0)/nineBoxData.length:0)],["Prom. Potencial",fmt(nineBoxData.length?nineBoxData.reduce((s,u)=>s+u.potScore,0)/nineBoxData.length:0)]].map(([label,val],i)=>(
    ...
    <Card style={{marginBottom:20}}>
      <div style={{display:"grid",gridTemplateColumns:"40px repeat(3,1fr)",gridTemplateRows:"auto repeat(3,1fr) auto",gap:0,minHeight:480}}>
        ...
        {[0,1,2].map(row=>[0,1,2].map(col=>{
          ...
          const bg=NINE_BOX_LABELS.cellColors[row][col];
          const txtColor=NINE_BOX_LABELS.cellTextColors[row][col];
          ...
          >
            <div style={{display:"flex",justifyContent:"space-between",alignItems:"flex-start",marginBottom:8}}>
              <div style={{fontSize:11,fontWeight:700,color:txtColor,textTransform:"uppercase",letterSpacing:"0.03em",lineHeight:1.3}}>{cellInfo.label}</div>
              <div style={{background:"rgba(255,255,255,0.6)",borderRadius:20,padding:"2px 8px",fontSize:18,fontWeight:800,color:txtColor,minWidth:28,textAlign:"center"}}>{cellUsers.length}</div>
            </div>
    ...
    {selectedCell&&(<Card title={`${NINE_BOX_LABELS.cells[selectedCell.row][selectedCell.col].label} — Detalle`} icon="👥" badge={`${grid[selectedCell.row][selectedCell.col].length} personas`} style={{marginBottom:20}}>
      ...
      {grid[selectedCell.row][selectedCell.col].length===0?
        <p style={{color:C.textLight,fontSize:13,textAlign:"center",padding:20}}>No hay personas en este cuadrante</p>:
        <div style={{overflowX:"auto"}}><table style={{width:"100%",borderCollapse:"collapse",fontSize:12}}>
          <thead><tr style={{borderBottom:`2px solid ${C.border}`}}>
            <th style={{textAlign:"left",padding:"10px 12px",color:C.textSec,fontWeight:600}}>Nombre</th>
            <th style={{textAlign:"center",padding:"10px 12px",color:C.textSec,fontWeight:600}}>Desempeño</th>
            <th style={{textAlign:"center",padding:"10px 12px",color:C.textSec,fontWeight:600}}>Potencial</th>
            <th style={{textAlign:"center",padding:"10px 12px",color:C.textSec,fontWeight:600}}>Acción</th>
          </tr></thead>
          <tbody>{grid[selectedCell.row][selectedCell.col].map((u,i)=>(
            <tr key={i} style={{borderBottom:`1px solid ${C.borderLight}`,background:i%2===0?C.white:C.bg}}>
...
const NineBoxScatter=({data,max,onSelectUser})=>{
  const w=600,h=400,pad={top:20,right:30,bottom:50,left:55};
...
const dimAvgs=useMemo(()=>{
  ...
  data.comp.forEach(r=>{
    ...
    // Normalize: trim + collapse spaces + lowercase for grouping
    const k=norm((r[COL.dimension]||"").trim().replace(/\s+/g," "));
    const p=parseFloat(r[COL.puntaje]);
    if(u&&k&&!isNaN(p)){
      if(!perPersonDim[u])perPersonDim[u]={};
      if(!perPersonDim[u][k])perPersonDim[u][k]=[];
      perPersonDim[u][k].push(p);
    }
  });
  // Build normalized key → display name map (first seen wins)
  const dimDisplayName={};
  data.comp.forEach(r=>{
    const raw=(r[COL.dimension]||"").trim().replace(/\s+/g," ");
    const k=norm(raw);
    if(k&&!dimDisplayName[k])dimDisplayName[k]=raw;
  });
  const dimTotals={};
  Object.values(perPersonDim).forEach(dims=>{
    Object.entries(dims).forEach(([k,vals])=>{
      const avg=vals.reduce((a,b)=>a+b,0)/vals.length;
      if(!dimTotals[k])dimTotals[k]=[];
      dimTotals[k].push(avg);
    });
  });
  return Object.entries(dimTotals).map(([k,vals])=>({name:dimDisplayName[k]||k,avg:vals.reduce((a,b)=>a+b,0)/vals.length}));
},[data]);
...
function App(){
const[sheetId,setSheetId]=useState("");const[potSheetId,setPotSheetId]=useState("");const[loading,setLoading]=useState(false);const[error,setError]=useState(null);const[data,setData]=useState(null);const[config,setConfig]=useState(null);const[selectedUser,setSelectedUser]=useState(null);const[connected,setConnected]=useState(false);const[view,setView]=useState("overview");const[exporting,setExporting]=useState(false);
...
const[potDirWeights,setPotDirWeights]=useState({});
const[showPotField,setShowPotField]=useState(false);
const[potWarning,setPotWarning]=useState(null);
const tt=useTooltip();
...
const generatePDF=(ud,cfg,mp,dw,weightedScore,labels)=>{
  ...
  const lbl=getScoreLabel(weightedScore,labels);
  ...
  const setC=(hex)=>{const[r,g,b]=hexToRgb(hex);pdf.setTextColor(r,g,b);};
  const setF=(hex)=>{const[r,g,b]=hexToRgb(hex);pdf.setFillColor(r,g,b);};
  const setD=(hex)=>{const[r,g,b]=hexToRgb(hex);pdf.setDrawColor(r,g,b);};
  const checkPage=(need)=>{if(y+need>H-M){pdf.addPage();y=M;return true;}return false;};
  ...
  const radarCx=M+CW*0.25,radarCy=y+30,radarR=22;
  const compData=ud.competencias.map(c=>{const wc=computeWeightedComp(c.name,ud.compDetail,(v)=>remapScale(v,cfg,mp),dw);return{label:c.name,value:wc!=null?wc:sv(c.score)};});
  ...
  ud.competencias.forEach((comp)=>{
    ...
    const details=ud.compDetail[comp.name]?Object.entries(ud.compDetail[comp.name]):[];
...
const exportZip=async()=>{setExporting(true);setExportProgress("");try{const zip=new JSZip();
  ...
  const blob=await zip.generateAsync({type:"blob"});const url=URL.createObjectURL(blob);const a=document.createElement("a");a.href=url;a.download="reportes_evaluacion.zip";a.click();URL.revokeObjectURL(url);
}catch(e){console.error(e);alert("Error generando el ZIP");}setExporting(false);setExportProgress("");};

const has9Box=!!potData&&!!potConfig;
...
if(!connected){return(<div style={{minHeight:"100vh",background:C.bg,display:"flex",alignItems:"center",justifyContent:"center",padding:20,fontFamily:font}}>
  <div style={{maxWidth:460,width:"100%",background:C.white,borderRadius:16,border:`1.5px solid ${C.border}`,padding:36,textAlign:"center"}}>
    ...
    {!showPotField?
      ...
      <div style={{textAlign:"left",marginBottom:16,background:C.accentBg,borderRadius:10,padding:14,border:`1px solid #E9D5FF`}}>
        <div style={{display:"flex",justifyContent:"space-between",alignItems:"center",marginBottom:8}}>
          <label style={{fontSize:12,fontWeight:600,color:C.accent}}>🧩 Sheet de Potencial (9-Box)</label>
...
const navTabStyle=(v)=>({padding:"5px 14px",borderRadius:8,border:`1.5px solid ${view===v?C.primary:C.border}`,background:view===v?C.primaryBg:C.white,color:view===v?C.primary:C.textSec,fontSize:12,cursor:"pointer",fontFamily:font,fontWeight:600});
...
const header=(<div style={{background:C.headerBg,borderBottom:`1.5px solid ${C.headerBorder}`,padding:"12px 24px",display:"flex",alignItems:"center",justifyContent:"space-between",flexWrap:"wrap",gap:10}}>
  <div style={{display:"flex",alignItems:"center",gap:10}}>
    ...
    <div style={{display:"flex",gap:4}}>
      <button onClick={()=>{setView("overview");setSelectedUser(null);}} style={navTabStyle("overview")}>Resumen</button>
      {has9Box&&<button onClick={()=>{setView("ninebox");setSelectedUser(null);}} style={navTabStyle("ninebox")}>🧩 9-Box</button>}
    </div>
    {view==="individual"&&<button onClick={()=>{setView("overview");setSelectedUser(null);}} style={{padding:"4px 10px",borderRadius:6,border:`1px solid ${C.border}`,background:C.white,color:C.textSec,fontSize:11,cursor:"pointer",fontFamily:font,fontWeight:500}}>← Volver</button>}
  </div>
  <div style={{display:"flex",gap:8,alignItems:"center",flexWrap:"wrap"}}>
    <UserSearch users={users} selectedUser={selectedUser} onSelect={handleSelectUser}/>
    <button onClick={exportZip} disabled={exporting} style={{padding:"7px 14px",borderRadius:8,border:`1.5px solid ${C.border}`,background:C.white,color:C.text,fontSize:12,cursor:exporting?"wait":"pointer",fontWeight:600,fontFamily:font,display:"flex",alignItems:"center",gap:4,maxWidth:exporting?280:"none",overflow:"hidden",whiteSpace:"nowrap",textOverflow:"ellipsis"}}>
      {exporting?(exportProgress||"⏳ Generando..."):"📥 PDF ZIP"}
    </button>
...
return(<div style={{minHeight:"100vh",background:C.bg,fontFamily:font}}>
  ...
  <div style={{maxWidth:960,margin:"0 auto",padding:24}}>
    ...
    <div style={{display:"grid",gridTemplateColumns:"1fr 1fr",gap:16,marginBottom:20}}>
      <Card title="Competencias" icon="🎯"><div style={{display:"flex",justifyContent:"center"}}><RadarChart data={userData.competencias.map(c=>{const wc=computeWeightedComp(c.name,userData.compDetail,scaleVal,dirWeights);return{label:c.name,value:wc!=null?wc:scaleVal(c.score)};})} maxVal={mx} onHover={(e,d)=>tt.show(e,`${d.label}\n${fmt(d.value)}`)} onLeave={tt.hide}/></div></Card>
    ...
    <div style={{display:"grid",gridTemplateColumns:"1fr 1fr",gap:14,marginBottom:22}}>
      {userData.competencias.map((comp,i)=>{const wc=computeWeightedComp(comp.name,userData.compDetail,scaleVal,dirWeights);const compScore=wc!=null?wc:scaleVal(comp.score);return(<Card key={i}>
        <div style={{display:"flex",justifyContent:"space-between",alignItems:"center",marginBottom:14}}>
          <h4 style={{fontSize:13,fontWeight:700,color:C.text,margin:0,maxWidth:"55%"}}>{comp.name}</h4>
          <div style={{display:"flex",alignItems:"center",gap:8}}><DiffBadge value={comp.dif}/><span style={{fontSize:22,fontWeight:800,color:C.primary}}>{fmt(compScore)}</span></div>
        </div>
